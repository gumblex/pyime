...
DATA_PATH = os.path.join(os.path.dirname(
    os.path.abspath(__file__)), 'data/pinyin_rime.txt')
CACHE_PATH = os.path.join(os.path.dirname(
    os.path.abspath(__file__)), 'data/pinyin.cache')
# start init
t_start = time.time()
try:
    ...
    lines = []
    with open(DATA_PATH, encoding='utf-8') as txt:
        for line in txt:
            ...
            if len(py_array) <= 6:
                for ip in product(*list(zip(py_array, py_short))):
                    initial_py = "".join(ip)
                    py_freq[initial_py] = py_freq.get(
                        initial_py, 0) + max(int(log(freq)), 1)
                    if not initial_py in p2c:
                        p2c[initial_py] = []
                    p2c[initial_py].append((freq, word))

    p2c = dict((k, tuple(w[1] for w in sorted(v, reverse=True)))
               for k, v in p2c.items())
    total = sum(chn_freq.values())
    chn_freq = dict((k, log(float(v) / total))
                    for k, v in chn_freq.items())
    py_freq = dict((k, log(float(v) / total)) for k, v in py_freq.items())
...
def dp_cut(sentence, topK=3):
    ...
    for i in range(N - 1, -1, -1):
        path[i] = []
        for hop in range(i + 1, min(i + max_word_len + 1, N + 1)):
            if (sentence[i:hop] in py_freq) or (hop - i == 1):
                for pt in path[hop]:
                    path[i].append([sentence[i:hop]] + pt)
            path[i] = sorted(path[i], key=lambda L: sum(word_rank(x)
                                                        for x in L), reverse=True)[:topK]
    return path[0]
...
def all_combine(m, idx):
    tb = {'n': 0}
    all_index_list = sorted(all_combine_idx(m, idx, tb), key=lambda L: sum(
        chn_rank(m[i][j]) for i, j in enumerate(L)), reverse=True)
    if len(m) > 1:
        all_index_list = all_index_list[:MAX_SHOW_ENTRIES]
    for column in all_index_list:
        yield "\n".join(m[i][column[i]] for i in range(len(m)))
